...
export interface MultipartUploadHandle {
  /** Uploads a single part. Returns the ETag for use in complete(). */
  upload_part(part_number: number, data: Buffer): Promise<string>;

  /** Finalises the multipart upload, assembling parts in ascending order. */
  complete(parts: Array<{ ETag: string; PartNumber: number }>): Promise<void>;

  /** Aborts the upload and cleans up uploaded parts (best-effort). */
  abort(): Promise<void>;
}
